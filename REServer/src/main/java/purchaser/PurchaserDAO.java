...
public class PurchaserDAO {
    ...
    public List<Purchaser> getPurchasersByPostcode(String postcode) {
        List<Purchaser> out = new ArrayList<>();
        for (Document d : coll.find(Filters.eq("postcodes", postcode)).limit(MAX_RESULTS)) {
            out.add(toPurchaser(d));
        }
        return out;
    }

    /** Adds a postcode of interest. Returns false if invalid, full (5), already present, or purchaser missing. */
    public AddResult addInterest(String purchaserId, String postcode) {
        ...
        Document existing = coll.find(Filters.eq("_id", new ObjectId(purchaserId))).first();
    ...
    /** Bulk-insert N synthetic purchasers each with 0..5 random NSW postcodes. Returns inserted count. */
